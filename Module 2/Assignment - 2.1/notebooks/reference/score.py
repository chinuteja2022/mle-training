...
parser = argparse.ArgumentParser(
    description='To score the model'
    )

parser.add_argument('--train_test_loc',
...
# loading datasets
X_test_prepared = pd.read_csv(os.path.join("datasets", train_test_loc, 'X_test_prepared.csv'))
...
housing_prepared = pd.read_csv(os.path.join("datasets", train_test_loc, 'housing_prepared.csv'))
housing_prepared.drop(columns=housing_prepared.columns[0], axis=1, inplace=True)
housing_labels = pd.read_csv(os.path.join("datasets", train_test_loc, 'housing_labels.csv'))
